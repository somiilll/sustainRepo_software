...
async def get_responses(org_id: str, facility_ids: list = None, **kwargs) -> dict:
    """Fetch GRI filled values, submission statuses, and section progress."""
    section = kwargs.get("category") or ""
    keyword = kwargs.get("metric") or kwargs.get("entity_name") or ""

    # 1. Primary data — esg_responses
    ...
    submissions = await db.esg_response_submissions.find(
        sub_query, {"_id": 0, "question_key": 1, "status": 1, "submitted_by_user_name": 1, "submitted_at": 1}
    ).to_list(100)
    ...
    section_stats = await db.esg_responses.aggregate(section_pipeline).to_list(20)
