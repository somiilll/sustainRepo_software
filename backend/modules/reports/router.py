...
from modules.reports.contracts import (
    ...
    return [
        {
            "id": "ghg_inventory",
            "name": "GHG Inventory",
            "description": "ISO 14064-1 inventory for selected facilities and periods.",
            "category": "GHG",
            "status": "ready" if has_ghg else "unavailable",
            "available": has_ghg,
            "action_label": "Configure report" if has_ghg else None,
            "required_modules": ["GHG"],
        },
        {
            "id": "ai_executive_summary",
            "name": "Executive Emissions Summary",
            "description": "Decision-ready emissions summary for leadership review.",
            "category": "GHG",
            "status": "ready" if has_ghg else "unavailable",
            "available": has_ghg,
            "action_label": "Configure report" if has_ghg else None,
        ...
        },
        {
            ...
            "description": "Scope, category, facility, and reporting-period roll-up.",
            "category": "GHG",
            "status": "planned",
            "available": False,
            "action_label": None,
        ...
        },
        {
            "id": "kpi_progress",
            "name": "KPI Progress",
            "description": "ESG metric completion and target progress across reporting periods.",
            "category": "ESG",
            "status": "planned" if has_esg else "unavailable",
            "available": False,
            "action_label": None,
            "required_modules": ["ESG"],
        },
        {
            "id": "workflow_status",
            "name": "Workflow Status",
            "description": "Submission, review, and approval status by owner and framework.",
            "category": "ESG",
            "status": "planned" if has_esg else "unavailable",
            "available": False,
            "action_label": None,
        ...
        },
        {
            ...
            "description": "Evidence coverage and data-readiness view for internal reviews.",
...
async def get_mis_reporting_context(current_user: dict) -> tuple[Optional[dict], bool]:
    ...
    reports_enabled = module_access.get("reports", True)
    return organization, current_user.get("role") == "admin" and reports_enabled


@router.get("/reports/catalog", response_model=MISReportCatalogResponse)
async def get_mis_report_catalog(current_user: dict = Depends(get_current_user)):
    """Expose the V1 report catalog with permission-aware availability."""
...
@router.get("/reports/filter-schema", response_model=MISReportFilterSchemaResponse)
async def get_mis_report_filter_schema(current_user: dict = Depends(get_current_user)):
    ...
    return {
        "reporting_period_format": "YYYY-MM",
        "supports_financial_year": True,
        "supports_calendar_year": True,
        "facilities": facilities,
        "available_scopes": ["scope1", "scope2", "scope3", "biogenic"],
    }


@router.get("/reports/history", response_model=MISReportHistoryResponse)
async def get_mis_report_history(current_user: dict = Depends(get_current_user)):
    ...
    history = await db.mis_report_history.find(
        query,
        {"_id": 0},
