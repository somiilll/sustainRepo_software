...
async def create_facility_for_organization(facility_data: FacilityCreate, org_id: str, current_user: dict) -> FacilityResponse:
    """Shared validated facility persistence for admin and supplier setup routes."""
    ...
    if org:
        ...
        if current_facility_count >= max_facilities:
            raise HTTPException(status_code=400, detail=f"Maximum facility limit ({max_facilities}) reached for your organization. Contact your administrator.")
    existing = await db.facilities.find_one({"name": facility_data.name, "organization_id": org_id})
    if existing:
    ...
    facility_dict.update({"id": str(uuid.uuid4()), "organization_id": org_id, "created_at": datetime.now(timezone.utc).isoformat()})
    await db.facilities.insert_one(facility_dict)
    audit_logger = get_audit_logger()
    await audit_logger.log(action=AuditAction.CREATE, module=AuditModule.FACILITY, user_id=current_user["id"], user_email=current_user["email"], user_role=current_user.get("role", "admin"), organization_id=org_id, resource_id=facility_dict["id"], resource_name=facility_data.name, description=f"Created facility '{facility_data.name}'", new_values=facility_dict)
...
async def create_facility(facility_data: FacilityCreate, current_user: dict = Depends(get_admin_user)):
    ...
    return await create_facility_for_organization(facility_data, current_user["organization_id"], current_user)
