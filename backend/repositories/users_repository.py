"""
Users repository — DB access abstraction for the `users_esg` collection.

All DB queries that touch the `users_esg` collection should funnel through
this repository so:
...
class UsersRepository:
    """Thin async wrapper over the `users_esg` Mongo collection."""
    ...
    async def find_by_id(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        return await self._db.users_esg.find_one({"id": user_id}, {"_id": 0})
    ...
    async def find_by_email(self, email: str, exclude_deleted: bool = True) -> Optional[Dict[str, Any]]:
        ...
        return await self._db.users_esg.find_one(query, {"_id": 0})
    ...
    async def find_by_email_any(self, email: str) -> Optional[Dict[str, Any]]:
        ...
        return await self._db.users_esg.find_one({"email": email}, {"_id": 0})

    async def insert(self, user_dict: Dict[str, Any]) -> None:
        await self._db.users_esg.insert_one(user_dict)

    async def update(self, user_id: str, update: Dict[str, Any]) -> None:
        await self._db.users_esg.update_one({"id": user_id}, {"$set": update})

    async def delete(self, user_id: str) -> None:
        """Hard delete — used by the admin route. Soft-delete uses `update`."""
        await self._db.users_esg.delete_one({"id": user_id})
    ...
    async def list_active_users_in_org(self, org_id: str) -> List[Dict[str, Any]]:
        ...
        return await self._db.users_esg.find(query, {"_id": 0, "password_hash": 0}).to_list(1000)

    async def count_active_users_in_org(self, org_id: str) -> int:
        return await self._db.users_esg.count_documents({
