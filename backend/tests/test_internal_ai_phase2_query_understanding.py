...
def test_consumption_contract_is_explicit_and_uses_emission_records():
    plan = build_query_plan("Diesel consumption in July 2026", _intent(), _month(), _resolved_fuel())
...
def test_scope_emission_contract_is_not_kpi_metadata_lookup():
...
def test_methodology_contract_requires_relationship_sources():
    plan = build_query_plan(
...
def test_version_and_audit_queries_are_distinct():
    version = build_query_plan("Tell me version history for diesel in July 2026", _intent("version_history"), _month(), _resolved_fuel())
    audit = build_query_plan("Show calculation audit for diesel in July 2026", _intent("formula_calculation"), _month(), _resolved_fuel())
    assert version.query_type == QueryType.RECORD_VERSION_HISTORY
    assert audit.query_type == QueryType.CALCULATION_AUDIT_LOOKUP


def test_generic_scope_question_never_gains_an_unrelated_fuel_entity():
    plan = build_query_plan(
...
def test_ambiguous_fuel_is_never_planned_as_found_data():
    plan = build_query_plan("Fuel use in July 2026", _intent(fuel="Unknown blend"), _month(), {"status": "AMBIGUOUS", "canonical_value": None})
