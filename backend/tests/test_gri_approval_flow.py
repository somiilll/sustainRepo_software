...
both $set and $setOnInsert in the update to esg_responses.

Also verifies:
- Save with status=saved routes to approval queue when approver is assigned
- After approval, esg_responses has approval_status=approved
- organization_esg_responses has the approved value at responses.<question_key>
"""
import os
import uuid
import time
...
def setup_assignment(db):
    ...
    db.esg_responses.delete_many({"organization_id": ORG_ID, "question_key": QUESTION_KEY})
    db.esg_submissions.delete_many({"organization_id": ORG_ID, "question_key": QUESTION_KEY})
    db.organization_esg_responses.update_many(
        {"org_id": ORG_ID, "framework": "GRI", "reporting_year": REPORTING_PERIOD},
        {"$unset": {f"responses.{QUESTION_KEY}": ""}},
    )
...
class TestGRIApprovalFlow:
    ...
    def test_01_save_creates_pending_submission(self, headers):
    ...
    def test_02_approve_submission_no_error(self, headers):
    ...
    def test_03_esg_responses_has_approved_status(self, db):
        """After approval, esg_responses should have approval_status=approved and correct value."""
        # Small wait for propagation (not necessary since awaited, but safe)
        time.sleep(0.5)
        doc = db.esg_responses.find_one(
            {"organization_id": ORG_ID, "question_key": QUESTION_KEY, "reporting_period": REPORTING_PERIOD}
        )
        assert doc is not None, "esg_responses doc missing after approval"
    ...
    def test_04_organization_esg_responses_has_value(self, db):
        """organization_esg_responses should also have the response for tracker."""
        # NOTE: the save flow (submit_for_approval path) writes this doc too.
        doc = db.organization_esg_responses.find_one(
            {"org_id": ORG_ID, "framework": "GRI", "reporting_year": REPORTING_PERIOD}
        )
        assert doc is not None, "organization_esg_responses doc missing"
        responses = doc.get("responses", {})
        assert responses.get(QUESTION_KEY) == TestGRIApprovalFlow.approved_value, (
            f"expected value at responses.{QUESTION_KEY}, got: {responses.get(QUESTION_KEY)!r}"
        )

    def test_05_submission_marked_approved(self, db):
        doc = db.esg_submissions.find_one({"id": TestGRIApprovalFlow.submission_id})
