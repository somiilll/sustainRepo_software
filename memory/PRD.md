...
- **Report Structure (12 sections)**:
  1. Cover Page (Company Logo, Name, Report Title, Period, Generated Date)
  2. Executive Summary (10 KPIs + Key Observations)
  3. KPI Summary (All KPIs in printable layout)
  4. Emissions Section (GHG Trend + Scope Breakdown + Table)
  5. Energy Section (Energy Mix + Renewable/Intensity + Summary)
  ...
  10. Performance Summary (▲ Improved, ▼ Declined, ► Stable indicators)
  11. AI Insights (Rule-based, no LLM - max 5 observations)
  12. Appendix (Metric Definitions)
- **Libraries Added**: jspdf, html2canvas
- **Features**:
  - Auto pagination with page breaks for charts
  - Header with company name, report title, reporting period
  - Footer with "Generated by SustainRepo", "Confidential", page number, date
  - Charts captured from DOM at high resolution
  - Rule-based insights (no AI API calls)
