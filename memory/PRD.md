...
- **Custom Fuel `kl` canonicalization — latest:** Hardcoded Custom Fuel Quantity options now use Calc Engine’s lowercase `kl`; generated quantity-basis EF options consequently use `kgCO2/kl`, and generated density is stored/sent as `kg/kl` instead of `kg/kL`. Create, Edit, calculation payloads, persistence, and legacy hydration normalize prior `kL` and `kg/kL` values. Heat-basis `TJ/kL`/`MJ/kL` dropdown keys remain unchanged so the existing `TJ/kL` compound catalog entry is not broken. **NOT TESTED** per the user’s standing instruction.
