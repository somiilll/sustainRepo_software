...
- **September 15, 2026 — Direct-save decision-tree defaults:** Fixed Scope 3 Spend direct-save formula resolution by setting the existing normalized currency-conversion choice (`ppp_inflation` or `standard`) before the calculation engine resolves its decision tree, rather than afterwards. The selected default is persisted on the OCR row for auditability and is used for the period-aware conversion inputs. Added the same pre-resolution preparation for Scope 1 Stationary and Mobile Combustion: direct OCR saves default to quantity-basis emission factors and derive the required `mass`/`volume` branch from the selected canonical unit (for example kg → mass; L/kL/m³ → volume). Unknown combustion units stop with a specific correction message instead of selecting an unsafe formula. The final GHG record stores the resolved methodology and formula version as before. **NOT RUNTIME-TESTED** per the user’s standing instruction.
