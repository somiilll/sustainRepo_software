...
- **September 15, 2026 — OCR factor-aware edit cascade:** Replaced free-text OCR subcategory and unit editing with database-backed dependent dropdowns. Scope/category/method selections load canonical factors from `fuel_database`, `scope3_ef`, or Water ESG category configuration; selecting a factor sets its lookup, source database, IDs, NAICS metadata, and only its allowed units. Valid extracted factor/unit values remain selected through database IDs, exact values, or registered unit aliases. Incompatible method/category changes require a new factor and unit before save. Backend validation prevents tampered factor/unit combinations and preserves audit history/verified overrides. **NOT RUNTIME-TESTED** per the user’s standing instruction.
