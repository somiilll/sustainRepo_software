...
- **September 16, 2026 — C5 landfill taxonomy synchronization:** Replaced `master_taxonomy.json` Scope 3 > C5 > `subcategories.landfill` with the exact 61 material names represented by active `scope3_ef` `C5 - Waste Generated in Operations` Activity Based `… - Landfilled` factors. Legacy shorthand was removed and the taxonomy retains material bases because disposal method is extracted separately. **SOURCE-REVIEWED ONLY; NOT RUNTIME-TESTED** per the user’s standing instruction.
