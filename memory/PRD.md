...
- **September 15, 2026 — OCR calculation-method persistence:** Direct OCR-to-GHG saves now persist calculation decisions both as canonical top-level record fields and in `dynamic_field_values`, which is the audit/input structure consumed by the GHG UI. Scope 1 records retain `calculation_methodology` (including `using_heat_basis_ncv`) and Scope 3 records retain the normalized `calculation_method_scope3` (`activity_basis`, `spend_basis`, or `supplier_basis`). The same resolved values are retained on the reviewed OCR row before finalization. This is source-type independent and therefore covers Excel OCR as well as invoice/bill OCR. **NOT RUNTIME-TESTED** per the user’s standing instruction.
