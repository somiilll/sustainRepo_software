...
## Current Priorities
- **C7 evidence upload repair — latest:** Restored the visible **Evidence Document** upload control to C7 creation by passing it to the Step 4 component that actually renders the uploader. The previously disconnected control props went to Step 1 and were ignored. The selected C7 file is now also passed into the C7 submission orchestrator, so its URL and original filename reach both monthly and yearly C7 payloads. C7 contracts, MongoDB write paths, and response models now preserve `evidence_file_name` alongside `evidence_url`, allowing the stored attachment to retain its readable name. **NOT TESTED** per the user’s explicit instruction.
