...
- September 22, 2026 — Questionnaire Builder terminology and authoring controls now distinguish ESG **Section** (Environment, Social, Governance) from the persisted per-question **Category** (Policy, Reporting, Certification). The question ledger and single-question editor both write the new category field; existing questions safely default to Policy. Each top-level question visibly exposes a labeled **Subquestion** action, and its follow-up dialog keeps the inherited ESG Section immutable while allowing an independent question Category. Duplication preserves both fields. **SOURCE-REVIEWED ONLY; NO FUNCTIONAL TESTING** per user instruction.
