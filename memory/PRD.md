...
- **Custom Fuel `kl` canonicalization — latest:** All hardcoded Custom Fuel Quantity, CV, quantity-basis EF, and generated density options now use lowercase `kl` (`TJ/kl`, `MJ/kl`, `kgCO2/kl`, and `kg/kl`). Create, Edit, calculation payloads, persistence, and legacy hydration normalize prior `/kL` values. Calc Engine retains strict catalog matching but accepts the lowercase alias for an existing legacy uppercase compound key, keeping historical `TJ/kL` records compatible without broad case-insensitive unit matching. **NOT TESTED** per the user’s standing instruction.
