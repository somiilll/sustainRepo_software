...
## Current Priorities
- **C7 row-level evidence upload — latest:** Replaced the single form-level C7 attachment with an **Evidence** control on every employee-period row in the C7 ledger. Each monthly employee row supports multiple uploads, displays an attachment count, allows viewing and removal, and writes its evidence list inside that row’s `monthly_data`; yearly C7 employee entries receive the same control after their annual fields. C7 creation payload builders now persist the nested `evidences` list per employee/month or employee/year, while C7 edit normalization and yearly edit payloads retain it. The shared upload endpoint is reused with existing authorization and size validation. **NOT TESTED** per the user’s explicit instruction.
