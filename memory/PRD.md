...
- **Yearly custom-fuel methodology validation repair — latest:** Scope 1 edit validation and serialization now prioritize the active methodology selected in the edit form over a stale hidden `calculation_methodology` dynamic value. A yearly custom-fuel Carbon Composition edit therefore validates Carbon Content and Oxidation Factor instead of incorrectly requiring an Emission Factor from Heat/Quantity Basis. **NOT TESTED** per the user’s explicit instruction.
