...
- **September 16, 2026 — Global OCR save rules:** Added generic (non-organization) save rules for Scope 1, Scope 2, and Scope 3 OCR rows. At Scope 3 save time, an unresolved generic extracted `Diesel` or `Petrol` now selects the configured canonical activity only when that option is eligible for the row: `Diesel (100% mineral diesel)` or `Petrol (100% mineral petrol)`. Explicit factor selections and non-matching categories are preserved. C1/C2 NAICS behavior was intentionally deferred. **SOURCE-REVIEWED ONLY; NOT RUNTIME-TESTED** per the user’s standing instruction.
