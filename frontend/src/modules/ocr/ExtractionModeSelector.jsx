...
export const ExtractionModeSelector = ({ modes, value, onChange, disabled, compact = false }) => (
  <section className={compact ? '' : 'space-y-3'} aria-labelledby={compact ? undefined : 'ocr-mode-heading'} aria-label={compact ? 'Extraction mode' : undefined} data-testid="ocr-mode-section">
    ...
    <div className={compact ? 'flex w-[140px] rounded-full border border-slate-200 bg-white p-1 shadow-sm' : 'grid gap-3 sm:grid-cols-2'} role="radiogroup" aria-label="Extraction mode">
      {modes.map((mode) => {
        ...
        return (
          ...
          >
            <span className={`grid h-5 w-5 shrink-0 place-items-center rounded-full ${active ? 'bg-white/15 text-white' : 'bg-teal-50 text-teal-700'}`}>
