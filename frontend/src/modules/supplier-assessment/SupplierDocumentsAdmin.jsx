...
export default function SupplierDocumentsAdmin() {
  ...
  return <div className="space-y-7" data-testid="supplier-documents-admin-page">
    ...
    <div className="space-y-4" data-testid="published-supplier-agreements-list">{documents.length === 0 ? <p className="py-8 text-center text-sm text-stone-500" data-testid="supplier-agreements-empty-state">No documents have been published.</p> : documents.map((document) => <Card key={document.id} className="rounded-xl border-stone-200 bg-white shadow-[0_4px_18px_rgba(28,55,43,0.05)]" data-testid={`published-supplier-agreement-${document.id}`}><CardContent className="flex flex-col gap-4 px-5 py-4 md:flex-row md:items-center"><div className="flex min-w-0 flex-1 items-start gap-3"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-stone-400" aria-hidden="true" /><div className="min-w-0"><p className="text-base font-semibold text-stone-950" data-testid={`supplier-document-title-${document.id}`}>{document.title}</p><p className="mt-2 flex flex-wrap items-baseline gap-x-2 gap-y-1 text-xs text-stone-500" data-testid={`supplier-document-response-options-${document.id}`}><span className="shrink-0 font-medium text-stone-600">{document.response_mode === 'STATUS' ? 'Response options' : 'Response'}</span><span>{document.response_mode === 'STATUS' ? (document.response_options || []).join(' · ') : 'Acceptance required'}</span></p><p className="mt-2 flex items-center gap-1.5 text-xs text-stone-600" data-testid={`supplier-document-due-date-${document.id}`}><CalendarDays className="h-3.5 w-3.5 text-stone-500" aria-hidden="true" />Due {document.due_date ? new Date(`${document.due_date.slice(0, 10)}T12:00:00`).toLocaleDateString() : 'not set'}</p><p className="mt-3 flex items-center gap-1.5 text-xs text-stone-600" data-testid={`agreement-assigned-suppliers-${document.id}`}><Users className="h-3.5 w-3.5 text-stone-500" aria-hidden="true" /><span className="font-semibold text-stone-700">{document.assigned_supplier_count || 0}</span> supplier{document.assigned_supplier_count === 1 ? '' : 's'} assigned</p></div></div><div className="flex shrink-0 flex-wrap items-center gap-2 md:justify-end"><Button variant="outline" size="sm" onClick={() => viewDocument(document)} data-testid={`preview-supplier-agreement-${document.id}`}><Eye className="h-3.5 w-3.5 mr-1" />Preview</Button><Button variant="outline" size="sm" onClick={() => openDocumentAssignments(document)} data-testid={`manage-document-assignments-${document.id}`}><Users className="h-3.5 w-3.5 mr-1" />Manage suppliers</Button><Button variant="outline" size="sm" onClick={() => { setDueDateDialog(document); setDueDateDraft(document.due_date?.slice(0, 10) || ''); }} data-testid={`edit-document-due-date-${document.id}`}><Pencil className="h-3.5 w-3.5 mr-1" />Due date</Button><Button variant="outline" size="sm" className="h-8 border-rose-200 bg-white text-rose-600 hover:bg-rose-50 hover:text-rose-700" disabled={deletingId === document.id} onClick={() => setPendingDelete(document)} data-testid={`delete-supplier-agreement-${document.id}`}><Trash2 className="h-3.5 w-3.5 mr-1" />Delete</Button></div></CardContent></Card>)}</div>
