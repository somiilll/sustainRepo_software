...
import { ClipboardList, ArrowRight, Calendar, CheckCircle, Circle } from 'lucide-react';
    ...
    <div className="space-y-6" data-testid="supplier-esg-overview">
      ...
      {questionnaires.length === 0 ? <Card data-testid="supplier-esg-overview-empty"><CardContent className="py-12 text-center text-sm text-stone-500">No ESG questionnaires are assigned.</CardContent></Card> : questionnaires.map((questionnaire) => (
        <Card key={questionnaire.questionnaire_id} data-testid={`supplier-esg-questionnaire-${questionnaire.questionnaire_id}`}>
          <CardHeader><div className="flex flex-wrap items-start justify-between gap-4"><div><CardTitle className="flex items-center gap-2"><ClipboardList className="h-5 w-5 text-emerald-600" />{questionnaire.questionnaire_name}</CardTitle><CardDescription className="mt-2" data-testid={`supplier-esg-progress-${questionnaire.questionnaire_id}`}>{Math.round(questionnaire.completion_percent || 0)}% complete</CardDescription></div><div className="flex flex-wrap items-center gap-2">{questionnaire.due_date && <Badge variant="outline" className="border-amber-300 bg-amber-50 text-amber-900" data-testid={`supplier-esg-due-date-${questionnaire.questionnaire_id}`}><Calendar className="mr-1 h-3 w-3" />Due {new Date(questionnaire.due_date).toLocaleDateString()}</Badge>}{questionnaire.status === 'submitted' ? <Badge className="bg-emerald-100 text-emerald-800" data-testid={`supplier-esg-status-${questionnaire.questionnaire_id}`}><CheckCircle className="mr-1 h-3 w-3" />Submitted</Badge> : <Badge className={questionnaire.status === 'in_progress' ? 'bg-blue-100 text-blue-800' : 'bg-amber-100 text-amber-800'} data-testid={`supplier-esg-status-${questionnaire.questionnaire_id}`}>{questionnaire.status === 'in_progress' ? 'In progress' : 'Not started'}</Badge>}</div></div><Progress value={questionnaire.completion_percent || 0} className="mt-4 h-2" data-testid={`supplier-esg-progress-bar-${questionnaire.questionnaire_id}`} /></CardHeader>
