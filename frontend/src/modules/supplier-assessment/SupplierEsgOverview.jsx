...
import { SupplierModuleAccordion } from './components/SupplierModuleAccordion';
...
export default function SupplierEsgOverview() {
  ...
  return <div className="mx-auto max-w-5xl space-y-7 pb-10" data-testid="supplier-esg-overview">
    ...
    {questionnaires.length === 0 ? <div className="rounded-lg border border-dashed border-slate-300 bg-white p-10 text-center text-sm text-slate-500" data-testid="supplier-esg-overview-empty">No ESG questionnaires are assigned.</div> : <Accordion type="multiple" defaultValue={[]} className="space-y-3" data-testid="supplier-esg-questionnaire-accordion">
      {questionnaires.map((questionnaire) => <SupplierModuleAccordion key={questionnaire.questionnaire_id} value={questionnaire.questionnaire_id} title={questionnaire.questionnaire_name} description={questionnaire.due_date ? `Due ${new Date(questionnaire.due_date).toLocaleDateString()}` : 'Assigned ESG questionnaire'} progress={questionnaire.completion_percent} status={questionnaireStatus(questionnaire)} icon={ClipboardList} iconClassName="bg-indigo-50 text-indigo-700" testId={`supplier-esg-questionnaire-${questionnaire.questionnaire_id}`}>
