...
export default function SupplierTrainingAdmin() {
  ...
  return <div className="space-y-7" data-testid="training-admin-page">
    <div className="border-b border-stone-200 pb-5" data-testid="training-admin-header"><div className="flex items-center gap-3"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg border border-amber-200 bg-amber-50 text-amber-700 shadow-sm" data-testid="training-admin-heading-icon"><GraduationCap className="h-6 w-6" aria-hidden="true" /></div><h1 className="text-3xl font-bold text-emerald-950" data-testid="training-admin-heading">Supplier {trainingLabel}</h1></div></div>
    <div className="flex flex-col gap-3 rounded-xl border border-stone-200 bg-white p-4 shadow-[0_4px_18px_rgba(28,55,43,0.06)] md:flex-row md:flex-wrap md:items-center lg:flex-nowrap" data-testid="training-admin-controls">
      <div className="relative w-full md:w-[min(430px,100%)] md:flex-none" data-testid="training-admin-search-control"><Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-stone-400" aria-hidden="true" /><Input value={trainingSearch} onChange={(event) => setTrainingSearch(event.target.value)} placeholder={`Search ${trainingLabel.toLowerCase()}...`} className="h-10 border-stone-200 bg-white pl-10 shadow-none transition-[border-color,box-shadow] focus-visible:border-emerald-600 focus-visible:ring-2 focus-visible:ring-emerald-100" aria-label={`Search ${trainingLabel}`} data-testid="training-admin-search-input" /></div>
      <Button className="h-10 shrink-0 bg-emerald-800 text-white shadow-sm transition-[background-color,box-shadow,transform] hover:-translate-y-px hover:bg-emerald-900 hover:shadow-md" onClick={() => setShowTrainingForm(true)} data-testid="open-add-training-button"><Upload className="mr-2 h-4 w-4" />Add {trainingLabel}</Button>
      <div className="flex w-full flex-col gap-2 md:ml-auto md:w-auto md:flex-row md:items-center md:gap-3" data-testid="training-admin-period-control"><Label htmlFor="training-admin-reporting-period" className="flex shrink-0 items-center gap-2 text-sm font-medium text-stone-600" data-testid="training-admin-period-label"><CalendarDays className="h-4 w-4 text-emerald-700" aria-hidden="true" />Reporting period</Label><Select value={reportingPeriod} onValueChange={setReportingPeriod}><SelectTrigger id="training-admin-reporting-period" className="h-10 w-full border-stone-200 bg-stone-50 font-medium text-stone-800 shadow-none transition-[border-color,box-shadow] focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100 md:w-44" data-testid="training-admin-period-selector"><SelectValue /></SelectTrigger><SelectContent data-testid="training-admin-period-menu">{periods.map((period) => <SelectItem key={period} value={period} data-testid={`training-admin-period-option-${period}`}>{period}</SelectItem>)}</SelectContent></Select></div>
    </div>
