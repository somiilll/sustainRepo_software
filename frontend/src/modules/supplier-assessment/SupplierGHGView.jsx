...
export default function SupplierGHGView() {
  ...
  const openUnlock = async (supplier) => {
    try {
      ...
      setUnlockTarget({ ...supplier, periods: submittedPeriods });
      setUnlockPeriodKey(submittedPeriods[0].period_key);
    } catch (error) {
      toast.error(error.response?.data?.detail || 'Could not load submitted GHG periods');
    }
  };

  const unlockSupplierGhg = async () => {
    if (!unlockTarget || !unlockPeriodKey) return;
    setUnlocking(true);
    try {
      const period = unlockTarget.periods.find((item) => item.period_key === unlockPeriodKey);
      await axios.post(`${API}/supplier-assessment/suppliers/${unlockTarget.supplier_relationship_id}/emissions/submission-periods/${encodeURIComponent(unlockPeriodKey)}/unlock`, {}, { headers: getAuthHeader() });
      toast.success(`${unlockTarget.supplier_name} can now revise and resubmit ${period?.label || 'this period'}`);
  ...
  return (
    <div className="space-y-6" data-testid="supplier-ghg-view">
      ...
      {/* Header */}
      <div className="flex flex-col gap-4 border-b border-stone-200 pb-5 xl:flex-row xl:items-center xl:justify-between" data-testid="supplier-ghg-header">
        ...
        <div className="flex w-full flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center xl:w-auto xl:flex-nowrap" data-testid="supplier-ghg-controls">
          <div className="relative w-full sm:w-52 sm:flex-none" data-testid="supplier-ghg-search-control"><Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-stone-400" aria-hidden="true" /><Input placeholder="Search emissions..." value={search} onChange={(event) => setSearch(event.target.value)} className="h-10 rounded-full border-stone-200 bg-stone-50 pl-10 shadow-none transition-[border-color,box-shadow] focus-visible:border-emerald-600 focus-visible:ring-2 focus-visible:ring-emerald-100" aria-label="Search emissions" data-testid="supplier-ghg-search-input" /></div>
          <TabsList className="h-10 shrink-0 rounded-full border border-stone-200 bg-stone-50 p-1" data-testid="supplier-ghg-tab-list"><TabsTrigger value="supplier-summary" className="h-8 rounded-full px-3.5 text-stone-600 shadow-none transition-[background-color,color] hover:bg-white hover:text-stone-900 data-[state=active]:bg-emerald-800 data-[state=active]:text-white data-[state=active]:shadow-sm" data-testid="supplier-ghg-summary-tab">Emissions by Supplier</TabsTrigger><TabsTrigger value="logs" className="h-8 rounded-full px-3.5 text-stone-600 shadow-none transition-[background-color,color] hover:bg-white hover:text-stone-900 data-[state=active]:bg-emerald-800 data-[state=active]:text-white data-[state=active]:shadow-sm" data-testid="supplier-ghg-logs-tab">Logs</TabsTrigger></TabsList>
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:gap-2" data-testid="supplier-ghg-period-control"><label htmlFor="supplier-ghg-reporting-period" className="flex shrink-0 items-center gap-2 text-sm font-medium text-stone-600" data-testid="supplier-ghg-period-label"><CalendarDays className="h-4 w-4 text-emerald-700" aria-hidden="true" />Reporting period</label><Select value={reportingPeriod} onValueChange={setReportingPeriod}><SelectTrigger id="supplier-ghg-reporting-period" className="h-10 w-full rounded-full border-stone-200 bg-stone-50 font-medium text-stone-800 shadow-none transition-[border-color,box-shadow] focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100 sm:w-44" data-testid="supplier-ghg-period-selector"><SelectValue /></SelectTrigger><SelectContent data-testid="supplier-ghg-period-menu">{periods.map((period) => <SelectItem key={period} value={period} data-testid={`supplier-ghg-period-option-${period}`}>{period}</SelectItem>)}</SelectContent></Select></div>
        </div>
      </div>
      ...
      <Dialog open={Boolean(unlockTarget)} onOpenChange={(open) => !open && setUnlockTarget(null)}><DialogContent data-testid="unlock-supplier-ghg-dialog"><DialogHeader><DialogTitle data-testid="unlock-supplier-ghg-title">Unlock GHG reporting period</DialogTitle><DialogDescription data-testid="unlock-supplier-ghg-description">Choose the submitted month, quarter, or year to reopen for this supplier.</DialogDescription></DialogHeader><div className="space-y-2"><label className="text-sm font-medium" htmlFor="unlock-supplier-ghg-period" data-testid="unlock-supplier-ghg-period-label">Submitted period</label><Select value={unlockPeriodKey} onValueChange={setUnlockPeriodKey}><SelectTrigger id="unlock-supplier-ghg-period" data-testid="unlock-supplier-ghg-period-selector"><SelectValue /></SelectTrigger><SelectContent data-testid="unlock-supplier-ghg-period-menu">{(unlockTarget?.periods || []).map((period) => <SelectItem key={period.period_key} value={period.period_key} data-testid={`unlock-supplier-ghg-period-option-${period.period_key}`}>{period.label}</SelectItem>)}</SelectContent></Select></div><DialogFooter><Button variant="outline" onClick={() => setUnlockTarget(null)} data-testid="cancel-unlock-supplier-ghg-button">Cancel</Button><Button disabled={unlocking || !unlockPeriodKey} onClick={unlockSupplierGhg} data-testid="submit-unlock-supplier-ghg-button">{unlocking ? 'Unlocking…' : 'Unlock period'}</Button></DialogFooter></DialogContent></Dialog>
