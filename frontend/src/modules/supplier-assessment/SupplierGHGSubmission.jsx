...
const periodTone = (period) => period.status === 'submitted'
  ...
  : period.status === 'unlocked' ? 'border-amber-200 bg-amber-50 text-amber-800'
    : period.is_overdue ? 'border-red-200 bg-red-50 text-red-800' : 'border-blue-200 bg-blue-50 text-blue-800';
  ...
  return <div className="space-y-6" data-testid="supplier-ghg-submission-page">
    ...
    <Card className="overflow-hidden border-stone-200" data-testid="supplier-ghg-period-ledger">
      ...
      <CardContent className="p-0">
        {!state ? <p className="p-6 text-sm text-stone-500" data-testid="supplier-ghg-periods-loading">Loading submission periods…</p> : <Table data-testid="supplier-ghg-periods-table"><TableHeader><TableRow><TableHead>Period</TableHead><TableHead>Deadline</TableHead><TableHead>Status</TableHead><TableHead>Submitted</TableHead><TableHead className="text-right">Action</TableHead></TableRow></TableHeader><TableBody>{periods.map((period) => <TableRow key={period.period_key} data-testid={`supplier-ghg-period-${period.period_key}`}><TableCell className="font-medium text-stone-900" data-testid={`supplier-ghg-period-label-${period.period_key}`}>{period.label}</TableCell><TableCell data-testid={`supplier-ghg-period-deadline-${period.period_key}`}>{new Date(`${period.due_date}T00:00:00Z`).toLocaleDateString('en-GB')}</TableCell><TableCell><Badge variant="outline" className={periodTone(period)} data-testid={`supplier-ghg-period-status-${period.period_key}`}>{period.status === 'submitted' ? <><Lock className="mr-1 h-3 w-3" />Submitted · Locked</> : period.status === 'unlocked' ? 'Unlocked for resubmission' : period.is_overdue ? 'Overdue' : 'In progress'}</Badge>{period.unlock_requested_at && period.status === 'submitted' && <p className="mt-1 text-xs text-stone-500" data-testid={`supplier-ghg-period-unlock-requested-${period.period_key}`}>Unlock requested</p>}{period.status === 'unlocked' && period.supplier_instructions && <p className="mt-1 max-w-sm text-xs text-amber-800" data-testid={`supplier-ghg-period-instructions-${period.period_key}`}>{period.supplier_instructions}</p>}</TableCell><TableCell data-testid={`supplier-ghg-period-submitted-at-${period.period_key}`}>{period.submitted_at ? new Date(period.submitted_at).toLocaleDateString('en-GB') : '—'}</TableCell><TableCell className="text-right">{period.status === 'submitted' ? <Button variant="outline" size="sm" disabled={requestingKey === period.period_key} onClick={() => requestUnlock(period)} data-testid={`request-unlock-supplier-ghg-period-${period.period_key}`}>{requestingKey === period.period_key ? 'Requesting…' : 'Request unlock'}</Button> : <Button size="sm" disabled={submitting} onClick={() => { setSelectedPeriod(period); setVerificationAccepted(false); }} data-testid={`submit-supplier-ghg-period-${period.period_key}`}><Send className="mr-1.5 h-3.5 w-3.5" />{period.status === 'unlocked' ? 'Resubmit' : 'Submit'}</Button>}</TableCell></TableRow>)}</TableBody></Table>}</CardContent>
    </Card>
