...
export default function SupplierDocuments() {
  const { getAuthHeader } = useAuth(); const [documents, setDocuments] = useState([]); const [loading, setLoading] = useState(true); const [loadError, setLoadError] = useState(''); const [submittingId, setSubmittingId] = useState(null); const [selectedStatus, setSelectedStatus] = useState({}); const [pendingSubmission, setPendingSubmission] = useState(null);
  const loadDocuments = useCallback(async () => { setLoading(true); setLoadError(''); try { setDocuments((await axios.get(`${API}/supplier-assessment/my-assessment/documents`, { headers: getAuthHeader() })).data || []); } catch (error) { setDocuments([]); setLoadError(getUserFriendlyError(error, 'Unable to load documents. Please try again.', { preferFallback: true })); } finally { setLoading(false); } }, [getAuthHeader]);
  useEffect(() => {
    loadDocuments();
  ...
  const viewDocument = async (document) => { try { const response = await axios.get(`${API}/supplier-assessment/my-assessment/documents/${document.id}/view`, { headers: getAuthHeader() }); window.open(response.data.url, '_blank', 'noopener,noreferrer'); setDocuments((current) => current.map((item) => item.id === document.id ? { ...item, has_been_viewed: true } : item)); } catch (error) { toast.error(error.response?.data?.detail || 'Could not open document'); } };
  ...
  return <div className="space-y-8" data-testid="supplier-documents-page">
    ...
    {!loadError && <SupplierStatusInfographics title="Document status" counts={statusCounts} testId="supplier-documents" />}
    <section className="mt-8 grid gap-4" data-testid="supplier-documents-list">
      {loadError ? <LoadErrorState title="Unable to load documents" message={loadError} onRetry={loadDocuments} testId="supplier-documents-load-error" /> : loading ? <p className="py-10 text-center text-sm text-stone-500" data-testid="supplier-documents-loading">Loading documents…</p> : documents.length === 0 ? <Card data-testid="supplier-documents-empty-state"><CardContent className="py-12 text-center text-sm text-stone-500">No documents are required for this assessment.</CardContent></Card> : documents.map((document) => <Card key={document.id} data-testid={`supplier-document-card-${document.id}`}><CardHeader><div className="flex items-start justify-between gap-4"><div><CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5 text-emerald-700" />{document.title}</CardTitle><CardDescription className="mt-2">{document.original_filename}</CardDescription>{document.due_date && <p className="mt-2 text-xs font-medium text-stone-600" data-testid={`supplier-document-due-date-${document.id}`}>Due {new Date(`${document.due_date.slice(0, 10)}T12:00:00`).toLocaleDateString()}</p>}</div>{isDocumentOverdue(document) && <Badge className="bg-rose-100 text-rose-800" data-testid={`supplier-document-overdue-${document.id}`}>Overdue</Badge>}{document.selected_response || document.accepted ? <Badge className="bg-emerald-100 text-emerald-800" data-testid={`supplier-document-response-${document.id}`}><CheckCircle2 className="mr-1 h-3 w-3" />{document.selected_response || 'Accepted'}</Badge> : document.submission_status === 'reopened' ? <Badge className="bg-amber-100 text-amber-800" data-testid={`supplier-document-reopened-${document.id}`}>Unlocked for resubmission</Badge> : <Badge variant="outline" data-testid={`supplier-document-pending-${document.id}`}>Response required</Badge>}</div></CardHeader><CardContent className="flex flex-wrap gap-3"><Button variant="outline" onClick={() => viewDocument(document)} data-testid={`view-supplier-document-${document.id}`}>View document<ExternalLink className="ml-2 h-4 w-4" /></Button>{document.response_mode === 'STATUS' && !document.selected_response ? <><div className="flex flex-wrap gap-2">{document.response_options.map((option) => <Button key={option} variant={selectedStatus[document.id] === option ? 'default' : 'outline'} disabled={submittingId === document.id} onClick={() => setSelectedStatus((current) => ({ ...current, [document.id]: option }))} data-testid={`document-status-option-${document.id}-${option.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`}>{option}</Button>)}</div>{document.has_been_viewed && <Button disabled={submittingId === document.id || !selectedStatus[document.id]} onClick={() => requestStatusSubmission(document)} data-testid={`submit-document-status-${document.id}`}>{submittingId === document.id ? 'Submitting…' : 'Submit response'}</Button>}</> : document.response_mode === 'ACCEPTANCE' && !document.accepted && document.has_been_viewed && <Button onClick={() => setPendingSubmission({ document, type: 'acceptance' })} disabled={submittingId === document.id} data-testid={`accept-supplier-document-${document.id}`}>{submittingId === document.id ? 'Recording…' : 'Submit acceptance'}</Button>}</CardContent></Card>)}
    </section>
