...
export default function SupplierDashboard() {
  ...
  const ghgProgress = Number(modules.get('ghg')?.completion_percent ?? relationship.ghg_completion_percent ?? 0);
  const ghgComplete = ghgProgress >= 100;
  ...
  return <div className="mx-auto max-w-7xl space-y-7 pb-10" data-testid="supplier-dashboard">
    ...
    <section className="space-y-3" data-testid="supplier-assessment-modules">
      ...
      <div className="space-y-8" data-testid="supplier-module-panels">
        ...
        {modules.has('ghg') && <SupplierModulePanel title={modules.get('ghg').display_name} description={ghgComplete ? 'All required GHG periods are submitted and visible to your customer.' : ghgHasDraftEntries ? 'Draft entries are ready for review and submission.' : 'Start by adding your assigned Scope 1 or Scope 2 entries.'} progress={ghgProgress} status={statusBadge(ghgComplete ? 'submitted' : ghgHasDraftEntries || ghgProgress > 0 ? 'in_progress' : 'pending', 'supplier-ghg-status')} icon={Cloud} iconClassName="bg-emerald-50 text-emerald-700" shadowClassName="shadow-[0_10px_28px_rgba(16,185,129,0.18)] hover:shadow-[0_14px_34px_rgba(16,185,129,0.24)]" testId="supplier-ghg-module-panel" action={!ghgComplete && <Button variant="outline" onClick={() => navigate(ghgHasDraftEntries ? '/supplier-assessment/emissions' : '/ghg')} data-testid="supplier-ghg-action-button">{ghgHasDraftEntries ? 'Review & submit GHG' : 'Add GHG entries'}</Button>} />}
