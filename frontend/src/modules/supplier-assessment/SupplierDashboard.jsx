  ...
  return <div className="mx-auto max-w-7xl space-y-7 pb-10" data-testid="supplier-dashboard">
    ...
    <section className="space-y-3" data-testid="supplier-assessment-modules">
      ...
      <div className="space-y-6" data-testid="supplier-module-panels">
        ...
        {modules.has('esg') && questionnaires.map((questionnaire) => <SupplierModulePanel key={questionnaire.questionnaire_id} title={questionnaire.questionnaire_name} description={questionnaire.status === 'submitted' ? 'Response submitted and locked.' : 'Response open.'} progress={questionnaire.completion_percent} status={statusBadge(questionnaire.status === 'submitted' ? 'submitted' : questionnaire.status === 'in_progress' ? 'in_progress' : 'pending', `questionnaire-status-${questionnaire.questionnaire_id}`)} icon={ClipboardList} iconClassName="bg-indigo-50 text-indigo-700" testId={`questionnaire-card-${questionnaire.questionnaire_id}`} action={<Button variant="outline" onClick={() => navigate(`/supplier-assessment/questionnaire/${questionnaire.questionnaire_id}`)} data-testid={`open-questionnaire-${questionnaire.questionnaire_id}`}>{questionnaire.status === 'submitted' ? 'View response' : 'Continue questionnaire'}</Button>} />)}

        {modules.has('ghg') && <SupplierModulePanel title={modules.get('ghg').display_name} description={ghgSubmitted ? 'Your GHG submission is locked and visible to your customer.' : ghgHasDraftEntries ? 'Draft entries are ready for review and submission.' : 'Start by adding your assigned Scope 1 or Scope 2 entries.'} progress={modules.get('ghg').completion_percent} status={statusBadge(ghgSubmitted ? 'submitted' : ghgHasDraftEntries ? 'in_progress' : 'pending', 'supplier-ghg-status')} icon={Cloud} iconClassName="bg-emerald-50 text-emerald-700" testId="supplier-ghg-module-panel" action={!ghgSubmitted && <Button variant="outline" onClick={() => navigate(ghgHasDraftEntries ? '/supplier-assessment/emissions' : '/ghg')} data-testid="supplier-ghg-action-button">{ghgHasDraftEntries ? 'Review & submit GHG' : 'Add GHG entries'}</Button>} />}

        {modules.has('documents') && <SupplierModulePanel title={modules.get('documents').display_name} description={documents.length === 0 ? 'No documents are assigned.' : pendingDocuments.length === 0 ? 'All assigned documents are complete.' : `${pendingDocuments.length} document response${pendingDocuments.length === 1 ? '' : 's'} remaining.`} progress={modules.get('documents').completion_percent} status={statusBadge(documents.length > 0 && pendingDocuments.length === 0 ? 'completed' : pendingDocuments.length ? 'pending' : 'in_progress', 'supplier-documents-status')} icon={FileText} iconClassName="bg-cyan-50 text-cyan-700" testId="supplier-documents-module-panel" action={pendingDocuments.length > 0 && <Button variant="outline" onClick={() => navigate('/supplier-assessment/documents/review')} data-testid="supplier-documents-action-button">Review documents</Button>} />}

        {modules.has('training') && <SupplierModulePanel title={modules.get('training').display_name} description={trainings.length === 0 ? 'No training is assigned.' : completedTrainings.length === trainings.length ? 'All assigned training is complete.' : `${trainings.length - completedTrainings.length} training item${trainings.length - completedTrainings.length === 1 ? '' : 's'} remaining.`} progress={modules.get('training').completion_percent} status={statusBadge(trainings.length > 0 && completedTrainings.length === trainings.length ? 'completed' : trainings.length ? 'pending' : 'in_progress', 'supplier-training-status')} icon={GraduationCap} iconClassName="bg-amber-50 text-amber-700" testId="supplier-training-module-panel" action={trainings.length > completedTrainings.length && <Button variant="outline" onClick={() => navigate('/supplier-assessment/training')} data-testid="supplier-training-action-button">Open training</Button>} />}
      </div>
    </section>
