...
export default function SupplierDashboard() {
  ...
  const progressItems = [
    { id: 'revenue', label: 'Revenue', progress: revenueProgress, Icon: DollarSign, iconClassName: 'bg-blue-50 text-blue-700' },
    ...(modules.has('esg') ? [{ id: 'esg', label: 'ESG Questionnaire', progress: modules.get('esg').completion_percent, Icon: ClipboardList, iconClassName: 'bg-indigo-50 text-indigo-700' }] : []),
    ...(modules.has('ghg') ? [{ id: 'ghg', label: 'GHG Emissions', progress: modules.get('ghg').completion_percent, Icon: Cloud, iconClassName: 'bg-emerald-50 text-emerald-700' }] : []),
    ...(modules.has('documents') ? [{ id: 'documents', label: 'Documents', progress: modules.get('documents').completion_percent, Icon: FileText, iconClassName: 'bg-cyan-50 text-cyan-700' }] : []),
    ...(modules.has('training') ? [{ id: 'training', label: 'Training', progress: modules.get('training').completion_percent, Icon: GraduationCap, iconClassName: 'bg-amber-50 text-amber-700' }] : []),
  ];
  ...
  return <div className="mx-auto max-w-7xl space-y-7 pb-10" data-testid="supplier-dashboard">
    ...
    <section className="space-y-3" data-testid="supplier-assessment-modules">
      ...
      <div className="space-y-4" data-testid="supplier-module-panels">
        ...
        {modules.has('esg') && questionnaires.length === 0 && <SupplierModulePanel title="ESG Questionnaire" description="No questionnaire has been assigned yet." progress={0} status={statusBadge('pending', 'supplier-esg-empty-status')} icon={ClipboardList} iconClassName="bg-indigo-50 text-indigo-700" testId="supplier-esg-module-panel"><p className="text-sm text-slate-500">Your customer has not assigned an ESG questionnaire yet.</p></SupplierModulePanel>}
        {modules.has('esg') && questionnaires.map((questionnaire) => <SupplierModulePanel key={questionnaire.questionnaire_id} title={questionnaire.questionnaire_name} description="ESG Questionnaire" progress={questionnaire.completion_percent} status={statusBadge(questionnaire.status === 'submitted' ? 'submitted' : questionnaire.status === 'in_progress' ? 'in_progress' : 'pending', `questionnaire-status-${questionnaire.questionnaire_id}`)} icon={ClipboardList} iconClassName="bg-indigo-50 text-indigo-700" testId={`questionnaire-card-${questionnaire.questionnaire_id}`}>
          <div className="flex flex-wrap items-center justify-between gap-4"><div className="min-w-[220px] flex-1"><div className="flex justify-between text-xs text-slate-500"><span data-testid={`questionnaire-progress-${questionnaire.questionnaire_id}`}>{Math.round(questionnaire.completion_percent || 0)}% complete</span><span>{questionnaire.status === 'submitted' ? 'Locked' : 'Response open'}</span></div><Progress value={questionnaire.completion_percent || 0} className="mt-2 h-2" data-testid={`questionnaire-progress-bar-${questionnaire.questionnaire_id}`} /></div><Button variant="outline" onClick={() => navigate(`/supplier-assessment/questionnaire/${questionnaire.questionnaire_id}`)} data-testid={`open-questionnaire-${questionnaire.questionnaire_id}`}>{questionnaire.status === 'submitted' ? 'View response' : 'Continue questionnaire'}</Button></div>
        </SupplierModulePanel>)}

        {modules.has('ghg') && <SupplierModulePanel title={modules.get('ghg').display_name} description="Log emissions and submit the completed snapshot." progress={modules.get('ghg').completion_percent} status={statusBadge(ghgSubmitted ? 'submitted' : ghgHasDraftEntries ? 'in_progress' : 'pending', 'supplier-ghg-status')} icon={Cloud} iconClassName="bg-emerald-50 text-emerald-700" testId="supplier-ghg-module-panel"><div className="flex flex-wrap items-center justify-between gap-4"><p className="text-sm text-slate-600">{ghgSubmitted ? 'Your GHG submission is locked and visible to your customer.' : ghgHasDraftEntries ? 'Draft entries are ready for review and submission.' : 'Start by adding your assigned Scope 1 or Scope 2 entries.'}</p>{!ghgSubmitted && <Button variant="outline" onClick={() => navigate(ghgHasDraftEntries ? '/supplier-assessment/emissions' : '/ghg')} data-testid="supplier-ghg-action-button">{ghgHasDraftEntries ? 'Review & submit GHG' : 'Add GHG entries'}</Button>}</div></SupplierModulePanel>}

        {modules.has('documents') && <SupplierModulePanel title={modules.get('documents').display_name} description="Review and respond to assigned documents." progress={modules.get('documents').completion_percent} status={statusBadge(documents.length > 0 && pendingDocuments.length === 0 ? 'completed' : pendingDocuments.length ? 'pending' : 'in_progress', 'supplier-documents-status')} icon={FileText} iconClassName="bg-cyan-50 text-cyan-700" testId="supplier-documents-module-panel"><div className="flex flex-wrap items-center justify-between gap-4"><p className="text-sm text-slate-600">{documents.length === 0 ? 'No documents are assigned.' : pendingDocuments.length === 0 ? 'All assigned documents are complete.' : `${pendingDocuments.length} document response${pendingDocuments.length === 1 ? '' : 's'} remaining.`}</p>{pendingDocuments.length > 0 && <Button variant="outline" onClick={() => navigate('/supplier-assessment/documents/review')} data-testid="supplier-documents-action-button">Review documents</Button>}</div></SupplierModulePanel>}

        {modules.has('training') && <SupplierModulePanel title={modules.get('training').display_name} description="Complete the learning assigned by your customer." progress={modules.get('training').completion_percent} status={statusBadge(trainings.length > 0 && completedTrainings.length === trainings.length ? 'completed' : trainings.length ? 'pending' : 'in_progress', 'supplier-training-status')} icon={GraduationCap} iconClassName="bg-amber-50 text-amber-700" testId="supplier-training-module-panel"><div className="flex flex-wrap items-center justify-between gap-4"><p className="text-sm text-slate-600">{trainings.length === 0 ? 'No training is assigned.' : completedTrainings.length === trainings.length ? 'All assigned training is complete.' : `${trainings.length - completedTrainings.length} training item${trainings.length - completedTrainings.length === 1 ? '' : 's'} remaining.`}</p>{trainings.length > completedTrainings.length && <Button variant="outline" onClick={() => navigate('/supplier-assessment/training')} data-testid="supplier-training-action-button">Open training</Button>}</div></SupplierModulePanel>}
      </div>
    </section>
