...
const RankingMetric = ({ id, label, value, detail, Icon, tone = 'stone' }) => {
  ...
  return <Card className={`group overflow-hidden rounded-2xl border-l-4 ${theme.surface} transition-[transform,box-shadow] duration-200 hover:-translate-y-0.5`} data-testid={`supplier-ranking-${id}-metric`}><CardContent className="relative flex min-h-[164px] flex-col justify-between p-5"><span className={`absolute -bottom-14 -right-8 h-28 w-[85%] rotate-[-11deg] rounded-tl-[100%] ${theme.wave}`} aria-hidden="true" /><span className={`absolute -bottom-20 -right-2 h-28 w-[78%] rotate-[-11deg] rounded-tl-[100%] border-t border-current ${theme.accent} opacity-20`} aria-hidden="true" /><div className="relative flex items-center gap-3"><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${theme.icon}`} data-testid={`supplier-ranking-${id}-icon`}><Icon className="h-5 w-5" aria-hidden="true" /></span><p className={`text-xs font-semibold uppercase tracking-wide ${theme.accent}`}>{label}</p></div><div className="relative"><p className="text-4xl font-semibold text-slate-950" data-testid={`supplier-ranking-${id}-value`}>{value}</p><p className="mt-2 truncate text-sm text-slate-500" data-testid={`supplier-ranking-${id}-detail`}>{detail}</p></div></CardContent></Card>;
};
