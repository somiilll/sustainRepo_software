...
export default function SupplierTraining() {
  ...
  return <div className="space-y-6" data-testid="supplier-training-page">
    ...
    {items.length === 0 ? <Card className="border-amber-200 shadow-[0_10px_28px_rgba(245,158,11,0.18)]" data-testid="supplier-training-empty"><CardContent className="py-10 text-center text-sm text-stone-500">No trainings are assigned.</CardContent></Card> : items.map((item) => { const status = trainingStatus(item.status); return <Card className="border-amber-200 shadow-[0_10px_28px_rgba(245,158,11,0.18)] transition-[transform,box-shadow] duration-200 hover:-translate-y-0.5 hover:shadow-[0_14px_34px_rgba(245,158,11,0.24)]" key={item.assignment_id} data-testid={`supplier-training-${item.assignment_id}`}><CardHeader><div className="flex justify-between gap-4"><div><CardTitle className="flex gap-2"><BookOpen className="h-5 w-5 text-amber-700" />{item.title}</CardTitle><CardDescription>{item.description} · Version {item.version_number}</CardDescription></div><Badge className={status.className} data-testid={`supplier-training-status-${item.assignment_id}`}>{status.label}</Badge></div></CardHeader><CardContent><Button variant="outline" onClick={() => openViewer(item)} disabled={isOpening} data-testid={`open-training-viewer-${item.assignment_id}`}>{isOpening ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Play className="mr-2 h-4 w-4" />}Open training</Button></CardContent></Card>; })}
