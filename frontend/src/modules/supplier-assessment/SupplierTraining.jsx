...
export default function SupplierTraining() {
  ...
  return <div className="space-y-8" data-testid="supplier-training-page">
    ...
    <section className="mt-8 grid gap-4" data-testid="supplier-training-list">
      {items.length === 0 ? <Card data-testid="supplier-training-empty"><CardContent className="py-10 text-center text-sm text-stone-500">No trainings are assigned.</CardContent></Card> : items.map((item) => { const status = trainingStatus(item.status); return <Card key={item.assignment_id} data-testid={`supplier-training-${item.assignment_id}`}><CardHeader><div className="flex justify-between gap-4"><div><CardTitle className="flex gap-2"><BookOpen className="h-5 w-5 text-emerald-700" />{item.title}</CardTitle>{item.description && <CardDescription>{item.description}</CardDescription>}{item.due_date && <p className="mt-2 text-xs font-medium text-stone-600" data-testid={`supplier-training-due-date-${item.assignment_id}`}>Due {new Date(`${item.due_date.slice(0, 10)}T12:00:00`).toLocaleDateString()}</p>}{item.page_count ? <p className="mt-2 text-xs text-stone-500" data-testid={`supplier-training-page-progress-${item.assignment_id}`}>Page {item.highest_page_index || 0} of {item.page_count} reached · {Math.round(item.progress_percent || 0)}% complete</p> : <p className="mt-2 text-xs text-stone-500" data-testid={`supplier-training-media-progress-${item.assignment_id}`}>{Math.round(item.progress_percent || 0)}% complete</p>}</div><div className="flex items-center gap-2">{isTrainingOverdue(item) && <Badge className="bg-rose-100 text-rose-800" data-testid={`supplier-training-overdue-${item.assignment_id}`}>Overdue</Badge>}<Badge className={status.className} data-testid={`supplier-training-status-${item.assignment_id}`}>{status.label}</Badge></div></div></CardHeader><CardContent><Button variant="outline" onClick={() => openViewer(item)} disabled={isOpening} data-testid={`open-training-viewer-${item.assignment_id}`}>{isOpening ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Play className="mr-2 h-4 w-4" />}{item.highest_page_index ? 'Continue training' : 'Open training'}</Button></CardContent></Card>; })}
    </section>
