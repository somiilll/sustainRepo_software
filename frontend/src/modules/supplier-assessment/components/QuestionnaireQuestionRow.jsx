...
export const QuestionnaireQuestionRow = ({ question, index, sectionLabel, questionCategoryLabel, typeLabel, scoringLabel, importanceClass, onEdit, onDelete, onDrop }) => {
  ...
  return <div onDragOver={(event) => event.preventDefault()} onDrop={(event) => onDrop(event.dataTransfer.getData('text/plain'), question.id)} className="grid gap-3 border-b border-stone-100/80 py-4 last:border-0 md:grid-cols-[2rem_minmax(11rem,1fr)_6rem_6rem_6rem_7.5rem_6.5rem_7.5rem] md:items-center" data-testid={`question-${question.id}`}>
    <span draggable onDragStart={beginDrag} className="flex h-7 w-7 cursor-grab items-center justify-center text-sm font-medium text-stone-500 active:cursor-grabbing" data-testid={`question-reorder-handle-${question.id}`}>{index + 1}<span className="sr-only">Drag question {index + 1}</span></span>
