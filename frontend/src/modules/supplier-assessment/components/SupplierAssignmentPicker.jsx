...
export const SupplierAssignmentPicker = ({ selectedIds, onChange, getAuthHeader, testIdPrefix, reportingPeriod, label = 'Assign to suppliers' }) => {
  ...
  return <div className="space-y-3" data-testid={`${testIdPrefix}-supplier-picker`}>
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stone-200 pb-3"><Label>{label}</Label><label className="flex items-center gap-2 text-sm font-medium"><input type="checkbox" checked={visibleSelected} onChange={(event) => toggleVisible(event.target.checked)} data-testid={`${testIdPrefix}-select-visible-checkbox`} />Select all shown</label></div>
