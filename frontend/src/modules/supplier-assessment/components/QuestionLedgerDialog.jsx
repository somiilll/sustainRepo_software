...
const blankRow = () => ({ question_text: '', category: 'environment', response_type: 'yes_no', importance: 'medium', scoring_rule: 'boolean', required: true, options_text: '' });
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="flex max-h-[calc(100dvh-2rem)] max-w-6xl flex-col gap-0 overflow-hidden bg-white p-0" data-testid="question-ledger-dialog">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
        <div className="hidden grid-cols-[minmax(13rem,1fr)_8rem_8rem_8rem_8rem_8rem_minmax(11rem,1fr)_2.5rem] gap-3 pb-2 text-xs font-medium uppercase text-stone-500 md:grid"><span>Question</span><span>Category</span><span>Response</span><span>Importance</span><span>Scoring</span><span>Required</span><span>Dropdown options</span><span /></div>
        <div className="space-y-3" data-testid="question-ledger-rows">{rows.map((row, index) => <div key={index} className="grid gap-3 rounded-lg border border-stone-200 bg-white p-3 md:grid-cols-[minmax(13rem,1fr)_8rem_8rem_8rem_8rem_8rem_minmax(11rem,1fr)_2.5rem] md:items-center" data-testid={`question-ledger-row-${index}`}>
          ...
          <Select value={row.scoring_rule} onValueChange={(scoring_rule) => updateRow(index, { scoring_rule })}><SelectTrigger data-testid={`question-ledger-scoring-${index}`}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="boolean">Yes/No score</SelectItem><SelectItem value="higher_is_better">Higher is better</SelectItem><SelectItem value="lower_is_better">Lower is better</SelectItem><SelectItem value="target_based">Target based</SelectItem><SelectItem value="choice_mapping">Choice mapping</SelectItem><SelectItem value="manual">Manual review</SelectItem></SelectContent></Select>
