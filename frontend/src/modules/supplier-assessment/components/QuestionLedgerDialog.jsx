...
const LedgerCell = ({ label, children }) => <div className="min-w-0 space-y-1.5"><Label className="text-xs font-medium text-stone-500 2xl:sr-only">{label}</Label>{children}</div>;
    ...
    <DialogContent className="flex max-h-[calc(100dvh-2rem)] w-[96vw] max-w-7xl flex-col gap-0 overflow-hidden bg-white p-0" data-testid="question-ledger-dialog">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6"><QuestionImportanceGuide testId="question-ledger-importance-weighting-guide" />
        <div className="mt-4" data-testid="question-ledger-rows">
          <div className="mb-2 hidden grid-cols-[minmax(12rem,1.45fr)_7rem_8rem_6rem_minmax(16rem,1.15fr)_5rem_8rem_2.75rem] gap-3 px-4 text-[11px] font-semibold uppercase tracking-wide text-stone-500 2xl:grid" data-testid="question-ledger-column-headings"><span>Question</span><span>Category</span><span>Response type</span><span>Importance</span><span>Scoring & scores</span><span>Required</span><span>Evidence</span><span className="text-center">Action</span></div>
          <div className="space-y-3">{rows.map((row, index) => <div key={index} className="grid min-w-0 grid-cols-1 gap-3 rounded-lg border border-stone-200 bg-white p-4 shadow-sm sm:grid-cols-2 2xl:grid-cols-[minmax(12rem,1.45fr)_7rem_8rem_6rem_minmax(16rem,1.15fr)_5rem_8rem_2.75rem] 2xl:items-start" data-testid={`question-ledger-row-${index}`}>
            ...
            <LedgerCell label="Category"><Select value={row.category} onValueChange={(category) => updateRow(index, { category })}><SelectTrigger data-testid={`question-ledger-category-${index}`}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="environment">Environment</SelectItem><SelectItem value="social">Social</SelectItem><SelectItem value="governance">Governance</SelectItem></SelectContent></Select></LedgerCell>
            <LedgerCell label="Response type"><Select value={row.response_type} onValueChange={(response_type) => updateRow(index, { response_type, scoring_rule: defaultRule(response_type), options_text: response_type === 'dropdown' ? row.options_text : '' })}><SelectTrigger data-testid={`question-ledger-response-type-${index}`}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="yes_no">Yes / No</SelectItem><SelectItem value="numeric">Numeric</SelectItem><SelectItem value="percentage">Percentage</SelectItem><SelectItem value="text">Text</SelectItem><SelectItem value="dropdown">Dropdown</SelectItem></SelectContent></Select></LedgerCell>
