...
const QuestionAuthoringRow = ({ draft, index, onChange, onRemove, canRemove }) => {
  ...
  return <div className="border border-stone-200 bg-white p-4" data-testid={`question-ledger-draft-row-${index}`}>
    <div className="grid gap-3 xl:grid-cols-[minmax(13rem,1.3fr)_8rem_9rem_7rem_5.5rem_auto] xl:items-end">
      ...
      <div className="space-y-1.5"><Label className="text-xs font-medium text-stone-600">Scoring type</Label><Select value={draft.scoring_rule} onValueChange={(scoring_rule) => setField({ scoring_rule })}><SelectTrigger data-testid={`question-ledger-draft-scoring-type-${index}`}><SelectValue /></SelectTrigger><SelectContent>{(scoringOptions[draft.response_type] || scoringOptions.yes_no).map((option) => <SelectItem key={option.value} value={option.value} data-testid={`question-ledger-draft-scoring-option-${index}-${option.value}`}>{option.label}</SelectItem>)}</SelectContent></Select></div>
      <div className="space-y-1.5"><Label className="text-xs font-medium text-stone-600">Importance</Label><Select value={draft.importance} onValueChange={(importance) => setField({ importance })}><SelectTrigger data-testid={`question-ledger-draft-importance-${index}`}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="low">Low</SelectItem><SelectItem value="medium">Medium</SelectItem><SelectItem value="high">High</SelectItem></SelectContent></Select></div>
      <label className="flex h-10 items-center gap-2 text-sm text-stone-700"><Checkbox checked={draft.required} onCheckedChange={(required) => setField({ required: Boolean(required) })} data-testid={`question-ledger-draft-required-${index}`} />Required</label>
...
export const QuestionLedgerDialog = ({ open, onOpenChange, onSave, saving, questions = [] }) => {
  ...
  const saveAll = async () => { const saved = await onSave(drafts, { keepOpen: true }); if (saved) resetDrafts(activeSection, activeCategory); };
