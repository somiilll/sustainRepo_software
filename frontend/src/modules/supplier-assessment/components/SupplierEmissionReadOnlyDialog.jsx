...
const buildReadOnlyDraft = (emission) => {
  const dynamicValues = Object.fromEntries(Object.entries(emission.dynamic_field_values || {}).map(([key, value]) => [key, fieldValue(value)]));
  const data = emission.dynamic_field_values || {};
  return {
    values: {
      ...
      source_of_information: emission.source_of_information || '', record_source: emission.record_source || '', notes: emission.notes || '',
      ...
      process_names: emission.process_descriptions?.length ? emission.process_descriptions : (emission.process_names || []).map((name) => ({ name, description: '' })),
...
export default function SupplierEmissionReadOnlyDialog({ open, onOpenChange, emission, loading, onOpenEvidence, openingEvidenceKey }) {
  ...
  const dynamicInputFields = useMemo(() => Object.entries(emission?.dynamic_field_values || {})
    .filter(([key]) => !key.endsWith('_unit') && key !== 'biogenic_scope_selection')
    .map(([variable, value]) => ({ id: `read-only-${variable}`, variable, fieldKey: variable, label: titleCase(variable), fieldType: typeof fieldValue(value) === 'number' ? 'number' : 'text', expectedUnit: fieldUnit(value), required: false, isOverride: Boolean(value?.is_override) })), [emission]);
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] max-w-6xl overflow-y-auto" data-testid="supplier-emission-read-only-dialog"><DialogHeader><div className="flex items-start justify-between gap-4"><div><DialogTitle className="text-xl text-emerald-950">View Emission Record</DialogTitle><DialogDescription className="mt-1 flex items-center gap-1.5"><Lock className="h-3.5 w-3.5" />Supplier submission — read-only</DialogDescription></div><Button variant="ghost" size="icon" aria-label="Close emission record" onClick={() => onOpenChange(false)} data-testid="close-supplier-emission-view"><X className="h-4 w-4" /></Button></div></DialogHeader>{loading || !emission || !draft ? <div className="py-16 text-center text-sm text-stone-500" data-testid="supplier-emission-view-loading">Loading emission record…</div> : <EmissionEditForm draft={draft} onDraftChange={() => {}} editingEmission={emission} activitySearchTerm="" loadingScope3EF={false} loadingBiogenicCategories={false} isCalculatingEditEmployee={false} isEditLoading={false} editFormConfigLoading={false} dynamicInputFields={dynamicInputFields} effectiveCalculatedEmissions={calculated} isCalculating={false} isSaving={false} setActivitySearchTerm={() => {}} facilities={[{ id: draft.values.facility_id, name: emission.facility_name || 'Supplier facility', is_active: true }]} dynamicScopes={dynamicScopes} hasScope3Access={false} centralizedUnits={[]} fuelDatabase={[{ id: draft.values.fuel_id, fuel_name: emission.fuel_type || emission.custom_fuel_name || 'Recorded fuel', allowed_units: [draft.values.quantity_unit].filter(Boolean) }]} selectedFuel={{ id: draft.values.fuel_id, fuel_name: emission.fuel_type || emission.custom_fuel_name || 'Recorded fuel', allowed_units: [draft.values.quantity_unit].filter(Boolean) }} isEditC7EmployeeCommuting={false} editActiveMonths={[]} ModuleDynamicFieldsRenderer={() => null} getCategoriesForScope={[emission.category].filter(Boolean)} getFuelsForCategory={() => []} availableScope3Methods={[]} availableScope3ActivityTypes={[]} capabilities={{}} fieldOptions={{}} requiresSubcategory={false} availableSubcategories={[]} filteredScope3Activities={[]} availableQuantityUnits={[{ value: draft.values.quantity_unit, label: draft.values.quantity_unit }].filter((unit) => unit.value)} handleSubmit={(event) => event.preventDefault()} handleFuelSelect={() => {}} handleCategorySelect={() => {}} markFormDirty={() => {}} updateDynamicFieldValue={() => {}} getMethodLabel={titleCase} handleCalculateEditEmployeeMonth={() => {}} handleFileUpload={() => {}} handleRemoveEvidence={() => {}} handleDeleteExistingEvidence={() => {}} handleDeleteAllEvidences={() => {}} handleDialogChange={onOpenChange} hideSubmitButton assignedReportingPeriod={{ reporting_period: emission.reporting_period }} readOnly readOnlyEvidenceContent={evidenceContent} />}</DialogContent></Dialog>;
}