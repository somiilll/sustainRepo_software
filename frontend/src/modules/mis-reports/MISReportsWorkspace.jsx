...
function ExecutiveReportSections({ report }) {
  if (!report) return null;
  return <section className="space-y-6" data-testid="mis-executive-report">
    <div><p className="text-xs font-bold uppercase tracking-[0.16em] text-emerald-800">Executive summary</p><div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5">{report.kpis.map((kpi) => <Card key={kpi.label} className="rounded-md border-stone-200 shadow-sm" data-testid={`mis-executive-kpi-${kpi.label.replaceAll(' ', '-').toLowerCase()}`}><CardContent className="p-4"><p className="text-xs text-stone-500">{kpi.label}</p><p className="mt-1 font-mono text-lg font-bold">{formatValue(kpi.value)}</p><p className="text-xs text-stone-500">Previous {formatValue(kpi.previous)} · {kpi.change_pct === null ? 'No prior data' : `${kpi.change_pct > 0 ? '▲' : '▼'} ${Math.abs(kpi.change_pct)}%`}</p></CardContent></Card>)}</div></div>
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
      <MetricSection title="Energy performance" metrics={[["Total energy", report.energy.total, "MWh"], ["Renewable energy", report.energy.renewable_total, "MWh"], ["Non-renewable energy", report.energy.non_renewable_total, "MWh"], ["Renewable share", report.energy.renewable_pct, "%"]]} testId="mis-energy-performance" />
      <MetricSection title="Water performance" metrics={[["Water consumption", report.water.consumption, "KL"], ["Water discharge", report.water.discharge, "KL"], ["Water withdrawal", report.water.withdrawal, "KL"], ["Water recycled", report.water.recycled, "KL"], ["Recycle share", report.water.recycle_pct, "%"]]} testId="mis-water-performance" />
      <MetricSection title="Waste performance" metrics={[["Waste generated", report.waste.generated, ""], ["Waste disposed", report.waste.disposal, ""], ["Waste recovered", report.waste.recovered, ""], ["Recovery share", report.waste.recovery_pct, "%"]]} testId="mis-waste-performance" />
    </div>
    <Card className="rounded-md border-stone-200 shadow-sm" data-testid="mis-factual-insights"><CardContent className="p-5"><h2 className="font-semibold">Carbon insights</h2><ul className="mt-3 space-y-2 text-sm text-stone-700">{report.insights.length ? report.insights.map((insight) => <li key={insight}>• {insight}</li>) : <li>No material change was identified in the selected period.</li>}</ul></CardContent></Card>
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-2"><MetricSection title="Operational performance" metrics={[["LTIFR", report.operational_kpis.ltifr, ""], ["Account payable days", report.operational_kpis.account_payable_days, "days"], ["GHG intensity by production", report.operational_kpis.ghg_intensity, "kg CO2e / unit"], ["Energy intensity", report.operational_kpis.energy_intensity, "MWh / unit"], ["No. of incidents", report.operational_kpis.incident_count, ""]]} testId="mis-operational-performance" /><MetricSection title="Supplier assessment" metrics={[["Suppliers assessed", report.supplier_assessment.suppliers_assessed, ""], ["High risk", report.supplier_assessment.high_risk_suppliers, ""], ["Pending", report.supplier_assessment.pending_assessments, ""]]} testId="mis-supplier-assessment" /></div>
  </section>;
}
