...
export default function EmissionCategoriesChart({ data = [] }) {
  ...
  return (
    <div className="space-y-6" data-testid="emission-categories-chart">
      {chartData.map((category, index) => (
        <div key={`${category.scope}-${category.name}`} data-testid={`category-ranking-row-${index + 1}`}>
          <div className="mb-1 flex items-start justify-between gap-3">
            <div className="flex min-w-0 items-start gap-2">
              <span className="w-5 shrink-0 text-xs font-bold tabular-nums text-stone-400" data-testid={`category-ranking-rank-${index + 1}`}>{String(index + 1).padStart(2, '0')}</span>
