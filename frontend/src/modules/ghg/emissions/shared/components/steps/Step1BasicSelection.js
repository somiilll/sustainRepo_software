        ...
        <div className="space-y-3 mt-4 pb-6 border-b border-stone-200">
          ...
          {!useCustomFuel ? (
            ...
            <div className="space-y-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
              ...
              {/* Custom fuel methodology-specific fields */}
              {(() => {
                ...
                if (calcMethod === 'using_heat_basis_ncv') {
                  return (
                    <div className="space-y-3 pt-2 border-t border-amber-200">
                      <p className="text-xs text-amber-700 font-medium">Heat Basis (NCV) — Custom Fuel Inputs</p>
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                          <Label className="text-xs">Emission Factor <span className="text-red-500">*</span></Label>
                          <Input type="number" step="any" min="0" value={customEmissionFactor} onChange={(e) => setCustomEmissionFactor(e.target.value)} placeholder="e.g. 0.074" className="bg-white" data-testid="custom-ef-value" />
                        </div>
                        <div className="space-y-1">
                          <Label className="text-xs">EF Unit <span className="text-red-500">*</span></Label>
                          <select value={customEmissionFactorUnit} onChange={(e) => setCustomEmissionFactorUnit(e.target.value)} className="w-full h-10 bg-white border border-stone-200 rounded-lg px-3 text-sm" data-testid="custom-ef-unit">
                            <option value="tCO2/TJ">tCO₂/TJ</option>
                            <option value="tCO2/MJ">tCO₂/MJ</option>
                          </select>
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                          ...
                          <Input type="number" step="any" min="0" value={decisionFieldValues._customCV || ''} onChange={(e) => setDecisionFieldValues(prev => ({ ...prev, _customCV: e.target.value }))} placeholder="e.g. 0.0431" className="bg-white" data-testid="custom-cv-value" />
                        </div>
                        <div className="space-y-1">
                          <Label className="text-xs">CV Unit <span className="text-red-500">*</span></Label>
                          <select value={decisionFieldValues._customCVUnit || 'TJ/kg'} onChange={(e) => setDecisionFieldValues(prev => ({ ...prev, _customCVUnit: e.target.value }))} className="w-full h-10 bg-white border border-stone-200 rounded-lg px-3 text-sm" data-testid="custom-cv-unit">
                            {['TJ', 'MJ'].map(num => ALL_QTY_UNITS.map(den => (
                      ...
                      <div className="space-y-1">
                        ...
                        <select value={decisionFieldValues._customQtyUnit || 'kg'} onChange={(e) => setDecisionFieldValues(prev => ({ ...prev, _customQtyUnit: e.target.value }))} className="w-full h-10 bg-white border border-stone-200 rounded-lg px-3 text-sm" data-testid="custom-qty-unit">
                ...
                if (calcMethod === 'using_qty_basis_ef') {
                  return (
                    <div className="space-y-3 pt-2 border-t border-amber-200">
                      <p className="text-xs text-amber-700 font-medium">Qty Basis EF — Custom Fuel Inputs</p>
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                          <Label className="text-xs">Emission Factor <span className="text-red-500">*</span></Label>
                          <Input type="number" step="any" min="0" value={customEmissionFactor} onChange={(e) => setCustomEmissionFactor(e.target.value)} placeholder="e.g. 2.68" className="bg-white" data-testid="custom-ef-value" />
                        </div>
                        <div className="space-y-1">
                          <Label className="text-xs">EF Unit <span className="text-red-500">*</span></Label>
                          <select value={customEmissionFactorUnit} onChange={(e) => setCustomEmissionFactorUnit(e.target.value)} className="w-full h-10 bg-white border border-stone-200 rounded-lg px-3 text-sm" data-testid="custom-ef-unit">
                            {ALL_QTY_UNITS.map(u => <option key={`tCO2/${u}`} value={`tCO2/${u}`}>tCO₂/{u}</option>)}
                          </select>
                        </div>
                      </div>
                      <div className="space-y-1">
                ...
                if (calcMethod === 'using_carbon_composition') {
                  return (
                    <div className="space-y-3 pt-2 border-t border-amber-200">
                      <p className="text-xs text-amber-700 font-medium">Carbon Composition — Custom Fuel Inputs</p>
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                          <Label className="text-xs">Carbon Content (%) <span className="text-red-500">*</span></Label>
                          <Input type="number" step="any" min="0" max="100" value={decisionFieldValues._customCarbonContent || ''} onChange={(e) => setDecisionFieldValues(prev => ({ ...prev, _customCarbonContent: e.target.value }))} placeholder="e.g. 85" className="bg-white" data-testid="custom-carbon-content" />
                        </div>
                        <div className="space-y-1">
                          <Label className="text-xs">Oxidation Factor <span className="text-red-500">*</span></Label>
                          <Input type="number" step="any" min="0" max="1" value={decisionFieldValues._customOxidationFactor || ''} onChange={(e) => setDecisionFieldValues(prev => ({ ...prev, _customOxidationFactor: e.target.value }))} placeholder="e.g. 1" className="bg-white" data-testid="custom-oxidation-factor" />
                        </div>
                      </div>
                      <div className="space-y-1">
                        <Label className="text-xs">Quantity Unit <span className="text-red-500">*</span></Label>
