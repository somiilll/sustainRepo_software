...
export const YearlyMetricEditor = ({
  data,
  disabled,
  isSaving,
  metric,
  ...
  return (
    <section className="border border-stone-200 bg-white p-4 sm:p-5" data-testid={`${testIdPrefix}-section`}>
      <div className="flex flex-col gap-4 border-b border-stone-100 pb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="text-base font-semibold text-text-primary" data-testid={`${testIdPrefix}-period-label`}>
            {periodLabel}
          </h3>
          <p className="mt-1 text-xs text-text-muted">
            {frequency === 'monthly' ? 'Monthly entries with annual total' : 'Annual entry'}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-end gap-2">
          {isRevenue ? (
            <div className="space-y-1">
              ...
              <Select
                value={data.turnover_currency || 'INR'}
                onValueChange={(value) => onChange({ turnover_currency: value })}
                disabled={disabled}
              >
                <SelectTrigger className="h-9 w-24" data-testid={`${testIdPrefix}-currency-select`}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-1">
              ...
              <Input
                value={data.production_unit || ''}
                onChange={(event) => onChange({ production_unit: event.target.value })}
                placeholder="Unit"
                className="h-9 w-24"
                disabled={disabled}
                data-testid={`${testIdPrefix}-unit-input`}
              />
            </div>
          )}
          ...
          <div className="space-y-1">
            ...
            <Select
              value={frequency}
              onValueChange={(value) => onChange({ [frequencyField]: value })}
              disabled={disabled}
            >
              <SelectTrigger className="h-9 w-28" data-testid={`${testIdPrefix}-frequency-select`}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
      ...
      <div className="pt-4">
        {frequency === 'yearly' ? (
          <div className="max-w-md space-y-2">
            ...
            <Input
              type="number"
              value={data[yearlyField] || ''}
              onChange={(event) => onChange({ [yearlyField]: event.target.value })}
              placeholder={isRevenue ? 'Enter turnover / revenue' : 'Enter production quantity'}
              disabled={disabled}
              data-testid={`${testIdPrefix}-yearly-input`}
            />
          </div>
        ) : (
              ...
              {months.map((month) => (
                <div key={month} className="space-y-1">
                  ...
                  <Input
                    ...
                    className="h-9"
                    placeholder="0"
                    value={data[monthlyField]?.[month] || ''}
                    onChange={(event) => updateMonthlyValue(month, event.target.value)}
                    disabled={disabled}
                    data-testid={`${testIdPrefix}-${month.toLowerCase()}-input`}
                  />
                </div>
              ))}
            </div>
            <p className="text-sm font-medium text-text-secondary" data-testid={`${testIdPrefix}-monthly-total`}>
              Annual total: {isRevenue ? `${data.turnover_currency || 'INR'} ` : ''}
              {monthlyTotal.toLocaleString()}
              {!isRevenue ? ` ${data.production_unit || ''}` : ''}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="mt-5 flex justify-end">
        ...
        >
          ...
          {isSaving ? 'Saving' : `Save ${periodLabel}`}
        </Button>
      </div>
    </section>
  );
