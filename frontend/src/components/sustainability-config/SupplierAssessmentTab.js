...
const buildState = (config) => Object.fromEntries(MODULES.map((module) => {
  ...
  return [module.code, {
    ...
    allow_custom_fuels: Boolean(saved.allow_custom_fuels),
    allow_process_emissions: Boolean(saved.allow_process_emissions),
    allow_flaring: Boolean(saved.allow_flaring),
  }];
}));
    ...
    supplier_assessment: {
      modules: Object.fromEntries(MODULES.map((module) => {
        ...
        return [module.code, {
          ...
          ...(module.code === 'ghg' ? { scopes: current.scopes, allow_custom_fuels: current.allow_custom_fuels, allow_process_emissions: current.allow_process_emissions, allow_flaring: current.allow_flaring } : {}),
        }];
      })),
    },
  });
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {MODULES.map((module) => {
          ...
          return <section className="border border-stone-200 p-5" key={module.code} data-testid={`supplier-assessment-module-${module.code}`}>
            ...
            {module.code === 'ghg' && <div className="mt-4 space-y-4"><div><p className="text-xs font-medium text-stone-700">Enabled scopes</p><div className="mt-2 flex flex-wrap gap-4"><Label className="flex items-center gap-2 text-sm font-normal"><Checkbox checked={current.scopes.includes('scope1')} onCheckedChange={(checked) => updateModule('ghg', { scopes: checked ? [...new Set([...current.scopes, 'scope1'])] : current.scopes.filter((scope) => scope !== 'scope1') })} data-testid="supplier-assessment-ghg-scope1-checkbox" />Scope 1</Label><Label className="flex items-center gap-2 text-sm font-normal"><Checkbox checked={current.scopes.includes('scope2')} onCheckedChange={(checked) => updateModule('ghg', { scopes: checked ? [...new Set([...current.scopes, 'scope2'])] : current.scopes.filter((scope) => scope !== 'scope2') })} data-testid="supplier-assessment-ghg-scope2-checkbox" />Scope 2</Label></div></div><div><p className="text-xs font-medium text-stone-700">Additional supplier permissions</p><div className="mt-2 flex flex-wrap gap-4"><Label className="flex items-center gap-2 text-sm font-normal"><Checkbox checked={current.allow_custom_fuels} onCheckedChange={(allow_custom_fuels) => updateModule('ghg', { allow_custom_fuels })} data-testid="supplier-assessment-ghg-custom-fuels-checkbox" />Custom fuels</Label><Label className="flex items-center gap-2 text-sm font-normal"><Checkbox checked={current.allow_process_emissions} onCheckedChange={(allow_process_emissions) => updateModule('ghg', { allow_process_emissions })} data-testid="supplier-assessment-ghg-process-emissions-checkbox" />Process emissions</Label><Label className="flex items-center gap-2 text-sm font-normal"><Checkbox checked={current.allow_flaring} onCheckedChange={(allow_flaring) => updateModule('ghg', { allow_flaring })} data-testid="supplier-assessment-ghg-flaring-checkbox" />Flaring</Label></div></div></div>}
          </section>;
        })}
      </div>
    </Card>
